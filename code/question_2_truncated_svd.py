...
def plot4basis(u):
    for i in range(0,4):
        ...
        plotsol(rho,'Density, $\\rho$, basis %d' %(i+1),'..\\figure\\question_2\\Fig_truncated_svd_rho_vector_%d.png' %(i+1));
        plotsol(x_momentum,'X-momentum, $\\rho u$, basis %d' %(i+1),'..\\figure\\question_2\\Fig_truncated_svd_x_momentum_vector_%d.png' %(i+1));
    return 0;
...
def main():
    ...
    #note that from la.svd, both u and uh are mostly negative
    np.save('..\\output\\truncated_svd_u.npy',u);
    plotsemilog(s,'..\\figure\\question_2\\Fig_truncated_svd_Singular_Value.png');
    plot4basis(u);
    return 0;
